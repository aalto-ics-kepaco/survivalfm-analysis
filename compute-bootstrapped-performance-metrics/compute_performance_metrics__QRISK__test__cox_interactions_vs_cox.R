...
result_list  <- purrr::map_dfr(
  ...
  dplyr::filter(display_name %in% c("Linear terms (standard Cox)" , "Linear terms + QRISK3 interaction terms with age (standard Cox)"))
...
cox_interactions_path <- result_list %>% 
  filter(display_name == "Linear terms + QRISK3 interaction terms with age (standard Cox)") %>% 
  pull(predictions_path) %>% 
  str_remove("/m/cs/scratch/ukbiobank-kepaco/tmp-heli/")

df_new <- readRDS(paste0("../model-training/", cox_interactions_path))
...
df_combined <- tibble(
  ...
  new_model = "Cox regression with age interactions",
...
# Save results to CSV
df_combined %>%
  readr::write_csv("../performance-metrics/performance_metrics__QRISK__test__cox_interactions_vs_cox.csv")
