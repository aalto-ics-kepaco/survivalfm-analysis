...
result_list  <- purrr::map_dfr(
  ...
  dplyr::filter(display_name %in% c("Linear terms + QRISK3 interaction terms with age (standard Cox)" , "Linear terms + factorized interaction terms (survivalFM)"))
...
df_combined <- tibble(
  std_model = "Cox regression with age interactions",
...
# Save results to CSV
df_combined %>%
  readr::write_csv("../performance-metrics/performance_metrics__QRISK__test__survivalfm_vs_cox_interactions.csv")
