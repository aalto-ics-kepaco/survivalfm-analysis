...
result_files <- list.files("../model-training/result-lists", full.names = T)
...
result_list  <- purrr::map_dfr(
  ...
  dplyr::filter(
    ...
    str_detect(training_method, "Standard Cox regression|Standard Cox regression with interaction terms")
  )
...
metrics <- function(df) {
  ...
  cindex_cox_interactions <- survival::concordance(survival::Surv(times, status) ~ lp_cox_interactions, data = df, reverse = TRUE)$concordance
  delta_cindex <- cindex_cox_interactions - cindex_cox
  ...
  nri <- nricens::nricens(
    ...
    p.new = df$abs_risk_cox_interactions,
    cut = 0,
    t0 = 10,
    updown = "diff",
    niter = 1
  ...
  fit_cox_interactions <- survival::coxph(
    as.formula("survival::Surv(time = times, event = status) ~ lp_cox_interactions"),
    data = df
  )
  rsq_cox <- survMisc::rsq(fit_cox, sigD = NULL)
  rsq_cox_interactions <- survMisc::rsq(fit_cox_interactions, sigD = NULL)
  
  delta_rsq <- rsq_cox_interactions$mev - rsq_cox$mev
  
  return(
    c(
      cindex_cox = cindex_cox, 
      cindex_cox_interactions = cindex_cox_interactions, 
      rsq_cox = rsq_cox$mev,
      rsq_cox_interactions = rsq_cox_interactions$mev,
...
cache_path <- "cache/performance_metrics__test__cox_interactions_vs_cox"
...
df_combined <- 
  purrr::map2_dfr(
    ...
    .y = result_list %>% select(input_name, endpoint_name) %>% distinct() %>% pull(endpoint_name),
    .f = function(input_name, endpoint_name) {
      ...
      if (length(cox_path) == 0) {return(NULL)}
      
      df_cox <- readRDS(paste0("../model-training/", cox_path))
      
      # cox_interactions
      cox_interactions_path <- result_list %>% 
        filter(input_name == !!input_name, endpoint_name == !!endpoint_name, str_detect(training_method, "Standard Cox regression with interaction terms")) %>% 
        pull(predictions_path) %>% 
        str_remove("/m/cs/scratch/ukbiobank-kepaco/tmp-heli/")
      
      if (length(cox_interactions_path) == 0) {return(NULL)}
      
      df_cox_interactions <- readRDS(paste0("../model-training/", cox_interactions_path))
      
      df <-
        dplyr::full_join(
          df_cox_interactions %>% select(eid, status, times, lp_cox_interactions = lp, abs_risk_cox_interactions = abs_risk),
          df_cox %>% select(eid, status, times, lp_cox = lp, abs_risk_cox = abs_risk),
          by = c("eid", "status", "times")
        )
      
      stopifnot(nrow(df) == nrow(df_cox_interactions))
      stopifnot(nrow(df) == nrow(df_cox))
      
      rm(df_cox_interactions, df_cox)
      ...
      ci_cindex_cox_interactions <- boot::boot.ci(boot_res, type = "perc", index = 2) # C-index
      ci_rsq_cox <- boot::boot.ci(boot_res, type = "perc", index = 3) # R-sq
      ci_rsq_cox_interactions <- boot::boot.ci(boot_res, type = "perc", index = 4) # R-sq
      ...
      tibble(
        ...
        cindex_cox_interactions = boot_res$t0[2],
        cindex_cox_interactions_ci_lower= ci_cindex_cox_interactions$perc[4],
        cindex_cox_interactions_ci_upper = ci_cindex_cox_interactions$perc[5],
        rsq_cox = boot_res$t0[3],
        rsq_cox_ci_lower = ci_rsq_cox$perc[4],
        rsq_cox_ci_upper = ci_rsq_cox$perc[5],
        rsq_cox_interactions = boot_res$t0[4],
        rsq_cox_interactions_ci_lower = ci_rsq_cox_interactions$perc[4],
        rsq_cox_interactions_ci_upper = ci_rsq_cox_interactions$perc[5],
...
# Save results to CSV
df_combined %>%
  readr::write_csv("../performance-metrics/performance_metrics__test__cox_interactions_vs_cox.csv")
