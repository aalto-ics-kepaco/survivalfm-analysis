...
# Read relatedness data ---------------------------------------------------

df_relatedness <- readr::read_csv("~/ukbiobank-kepaco/helper-files/ukb_rel.dat")

df_centers <- data.table::fread("~/ukbiobank-kepaco/preprocessed-data/baseline_characteristics.tsv") %>% 
...
df_relatedness_test_train <- df_relatedness %>% 
  dplyr::mutate(
    country1 = ifelse(ID1 %in% scotland_eids, "Scotland", "England or Wales"),
    country2 = ifelse(ID2 %in% scotland_eids, "Scotland", "England or Wales")
  ) %>% 
...
cache_path <- "cache/performance_metrics__test_excluding_related__survivalfm_vs_cox"
...
df_combined <- 
  purrr::map2_dfr(
    ...
    .f = function(input_name, endpoint_name) {
      ...
      # Remove individuals from the test set who are related to individuals in England and Wales (train)
      
      df <- df %>% 
        filter(!eid %in% c(df_relatedness_test_train$ID1, df_relatedness_test_train$ID2))
...
# Save results to CSV
df_combined %>%
  readr::write_csv("../performance-metrics/performance_metrics__test_excluding_related__survivalfm_vs_cox.csv")
